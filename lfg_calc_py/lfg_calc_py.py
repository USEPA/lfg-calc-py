...
class LFG:
    ...
    def calculate_lfg_emissions(
        ...
        # T is the landfill lifespan
        for landfill_year in range(0, T):
            row = {"Year": year_init + landfill_year}

            for material in material_type_list:
                # Initial emissions for each year of landfill operation year is 0 for each material
                methane_annual = 0

                # x represents the year that the waste was landfilled
                for x in range(0, landfill_year):
                    # todo: skip years where input waste is 0
                    # calculate the emissions generated during a landfill operation year for the waste input
                    # during each year of operation up through the current landfill operation year
                    methane_by_input_waste_year = (
                        ...
                        * 16/12
                        * (exp(-self.return_material_decay_rates(material) * (landfill_year - x - 1))
                            - exp(-self.return_material_decay_rates(material) * (landfill_year - x)))
                    )
                ...
                log.info(f"{material} annual landfill emissions for year {landfill_year}: {methane_annual}")
